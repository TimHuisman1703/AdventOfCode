...
class Unit:
	...
	def copy(self):
		return Unit(self.id, self.x, self.y, self.side, self.health)
...
while 1:
	g = [j.copy() for j in backup[0]]
	units = {key: value.copy() for key, value in backup[1].items()}
	alive = backup[2].copy()
	steps = 0
	...
	while alive[0] and alive[1]:
		...
		for i in turns:
			...
			if best:
				best.health -= attack if side else 3
				if best.health <= 0:
					g[best.y][best.x] = 0
					alive[best.side] -= 1
					units.pop(best.id)
	
	if alive[1] == backup[2][1]:
		break
