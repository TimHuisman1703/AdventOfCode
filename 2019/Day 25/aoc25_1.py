...
while 1:
	...
	if instr == "brute force":
		instr = [ord(j) for j in "inv\n"]
		output, reason = intcode.run(instr)
		output = "".join(chr(j) for j in output)
		items = [j[2:] for j in output.split("\n")[2:-3]]

		for i in range(1 << len(items)):
			instr = ""
			for j in range(len(items)):
				instr += f"{['take', 'drop'][(i >> j) & 1]} {items[j]}\n"
			instr += "south\n"

			instr = [ord(j) for j in instr + "\n"]
			output, reason = intcode.run(instr)
			output = "".join(chr(j) for j in output)

			print(f"Tried {i} configuration(s)...")
