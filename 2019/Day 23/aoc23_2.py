...
intcode = [IntCode(l) for _ in range(N)]
to_nat = [0, 0]
from_nat = [0, 0]
...
while 1:
	for i in range(N):
		no_input = len(intcode[i].input) == 0
		if no_input:
			intcode[i].input = [-1]
		else:
			idle[i] = 0
		
		output, reason = intcode[i].execute()

		if no_input:
			if intcode[i].input == [-1]:
				intcode[i].input = []
			else:
				idle[i] += (idle[i] < TRIES)
		...
		if len(intcode[i].output) == 3:
			...
			addr, x, y = intcode[i].output
			intcode[i].output = []

			if addr == 255:
				to_nat = [x, y]
			else:
				intcode[addr].input += [x, y]
	
	if sum(idle) == TRIES * N:
		print(f"Sent from NAT to computer 0: {to_nat}")

		if to_nat[1] == from_nat[1]:
			print(to_nat[1])
			break
		
		from_nat = to_nat
		intcode[0].input += from_nat